...
public class AudioManager : MonoBehaviour
{
	...
	void Awake()
	{
		...
		else
		{
			...
			sfxVolumePercent = PlayerPrefs.GetFloat("sfx vol", 0.7f);
			musicVolumePercent = PlayerPrefs.GetFloat("music vol", 0.3f);
		}
	}
    ...
    {
		//리스너 추가
		AudioListenerObj = new GameObject("SoundListener");
		AudioListenerObj.AddComponent<AudioListener>();
		Target = GameObject.Find("Player").gameObject;
		
		//리스너 위치 찾아가기
		AudioListenerObj.transform.position = Target.transform.position;
		AudioListenerObj.transform.parent = Target.transform;
	}
